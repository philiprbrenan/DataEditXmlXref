...
sub renewXref($)                                                                #P Renew a cross referencer unpacked from a file
 {my ($xref) = @_;                                                              # Cross referencer
  newXref();                                                                    # Create cross referencer methods
  bless $_[0];                                                                  # Bless the parameterized reference
 }
...
sub xref2(%)                                                                    #P Check the cross references in a set of Dita files held in  L<inputFolder|/inputFolder> and report the results in the L<reports|/reports> folder. The possible attributes are defined in L<Data::Edit::Xml::Xref|/Data::Edit::Xml::Xref>
 {my (%attributes) = @_;                                                        # Attributes of cross referencer
  ...
  $xref->fixedFolderTemp //= temporaryFolder;                                   # Preserve this temporary value across session instances
...
sub xref(%)                                                                     # Check the cross references in a set of Dita files held in L<inputFolder|/inputFolder> and report the results in the L<reports|/reports> folder. The possible attributes are defined in L<Data::Edit::Xml::Xref|/Data::Edit::Xml::Xref>.
 {my (%attributes) = @_;                                                        # Cross referencer attribute value pairs
  ...
  renewXref $x;                                                                 # Return blessed results
 }
...
# Conversely, existing map to map references should be left alone
  ...
  if (my $S = -p $x)                                                            # Source after any changes
   {if ($S ne $s)                                                               # Write any changes - seems to be slightly faster than not checking
     {if (onAwsButNotSessionLeader)                                             # Write output to temporary folder regardless so it can be copied enmasse back to the session leader
...
sub fixReferencesParallel($$)                                                   #P Fix the references in one file
 {my ($xref, $file) = @_;                                                       # Cross referencer, file to fix
  ...
  newXref(fixedRefsGood => $xref->fixedRefsGood,
          fixedRefsBad  => $xref->fixedRefsBad);
 };
...
sub fixReferencesResults($@)                                                    #P Consolidate the results of fixing references.
 {my ($Xref, @results) = @_;                                                    # Cross referencer, results from fixReferencesInParallel
  my $xref = onAwsButNotSessionLeader ? newXref() : renewXref($Xref);           # Reconstitute
  for my $x(@results)
...
END
  ...
  if (my @files = sort keys %{$xref->fixRefs})                                  # Fix files if requested
   {awsParallelProcessFiles $xref,                                              # Fix files in parallel
    ...
    if (onAwsButNotSessionLeader)                                               # Recover fixed files from other instances
     {my $s = $xref->fixedFolderTemp;
      my $t = $xref->fixedFolder // $xref->inputFolder;
      awsParallelGatherFolder($s);                                              # Recover fixed files
      copyFolder($s, $t);                                                       # Merge them into the target area
     }
   }
...
sub analyzeOneFileResults($@)                                                   #P Merge a list of cross reference results into the first cross referencer in the list
 {my ($Xref, @x) = @_;                                                          # Cross referencer to merge into, other cross referencers
  ...
  my $xref = onAwsButNotSessionLeader ? newXref() : renewXref($Xref);           # Create methods
...
4 L<analyzeOneFileResults|/analyzeOneFileResults> - Merge a list of cross reference results into the first cross referencer in the list

5 L<analyzeOneFileParallel|/analyzeOneFileParallel> - Analyze one input file
...
43 L<createWordsToFilesTest|/createWordsToFilesTest> - Index words to file

44 L<editXml|/editXml> - Edit an xml file retaining any existing XML headers and lint trailers

45 L<externalReference|/externalReference> - Check for an external reference

46 L<fixFilesGB|/fixFilesGB> - Rename files to the L<GB Standard|http://metacpan.org/pod/Dita::GB::Standard>

47 L<fixingRun|/fixingRun> - A fixing run fixes problems where it can and thus induces changes which might make the updated output different from the incoming source.

48 L<fixOneFileGB|/fixOneFileGB> - Fix one file to the Gearhart-Brenan standard

49 L<fixReferences|/fixReferences> - Fix just the file containing references using a number of techniques and report those references that cannot be so fixed.

50 L<fixReferencesInOneFile|/fixReferencesInOneFile> - Fix one file by moving unresolved references to the xtrf attribute

51 L<fixReferencesParallel|/fixReferencesParallel> - Fix the references in one file

52 L<fixReferencesResults|/fixReferencesResults> - Consolidate the results of fixing references.

53 L<formatTables|/formatTables> - Using cross reference B<$xref> options and an array of arrays B<$data> format a report as a table using B<%options> as described in L<Data::Table::Text::formatTable> and L<Data::Table::Text::formatHtmlTable>.

54 L<hashOfCountsToArray|/hashOfCountsToArray> - Convert a B<$hash> of {key} = count to an array so it can be formatted with L<formatTables>

55 L<loadInputFiles|/loadInputFiles> - Load the names of the files to be processed

56 L<newXref|/newXref> - Create a new cross referencer

57 L<oneBadRef|/oneBadRef> - Check one reference and return the first error encountered or B<undef> if no errors encountered.

58 L<oxygenProjectFileMetaData|/oxygenProjectFileMetaData> - Meta data for the oxygen project files

59 L<removeStringsFromData|/removeStringsFromData> - Given a data structure, remove the specified strings from it

60 L<removeUnusedIds|/removeUnusedIds> - Remove ids that do are not mentioned in any href or conref in the corpus regardless of the file component of any such reference.

61 L<reportAttributeCount|/reportAttributeCount> - Report attribute counts

62 L<reportAttributeNameAndValueCounts|/reportAttributeNameAndValueCounts> - Report attribute value counts

63 L<reportConRefMatching|/reportConRefMatching> - Report conref matching

64 L<reportDocTypeCount|/reportDocTypeCount> - Report doc type count

65 L<reportDuplicateIds|/reportDuplicateIds> - Report duplicate ids

66 L<reportDuplicateTopicIds|/reportDuplicateTopicIds> - Report duplicate topic ids

67 L<reportEmptyTopics|/reportEmptyTopics> - Report empty topics

68 L<reportExteriorMaps|/reportExteriorMaps> - Maps that are not referenced by any other map

69 L<reportExternalXrefs|/reportExternalXrefs> - Report external xrefs missing other attributes

70 L<reportFileExtensionCount|/reportFileExtensionCount> - Report file extension counts

71 L<reportFileTypes|/reportFileTypes> - Report file type counts - takes too long in series

72 L<reportFixRefs|/reportFixRefs> - Report of hrefs that need to be fixed

73 L<reportGuidHrefs|/reportGuidHrefs> - Report on guid hrefs

74 L<reportGuidsToFiles|/reportGuidsToFiles> - Map and report guids to files

75 L<reportHrefUrlEncoding|/reportHrefUrlEncoding> - href needs url encoding

76 L<reportIdRefs|/reportIdRefs> - Report the number of times each id is referenced

77 L<reportImages|/reportImages> - Reports on images and references to images

78 L<reportLtGt|/reportLtGt> - Report items found between &lt; and &gt;

79 L<reportMaxZoomOut|/reportMaxZoomOut> - Text located via Max Zoom In

80 L<reportMd5Sum|/reportMd5Sum> - Report files with identical md5 sums

81 L<reportNoHrefs|/reportNoHrefs> - Report locations where an href was expected but not found

82 L<reportOlBody|/reportOlBody> - ol under body - indicative of a task

83 L<reportOtherMeta|/reportOtherMeta> - Advise in the feasibility of moving othermeta data from topics to bookmaps assuming that the othermeta data will be applied only at the head of the map rather than individually to each topic in the map.

84 L<reportParseFailed|/reportParseFailed> - Report failed parses

85 L<reportPublicIds|/reportPublicIds> - Report public ids in use

86 L<reportReferencesFromBookMaps|/reportReferencesFromBookMaps> - Topics and images referenced from bookmaps

87 L<reportRequiredCleanUps|/reportRequiredCleanUps> - Report required clean ups

88 L<reportSimilarTopicsByTitle|/reportSimilarTopicsByTitle> - Report topics likely to be similar on the basis of their titles as expressed in the non Guid part of their file names

89 L<reportSimilarTopicsByVocabulary|/reportSimilarTopicsByVocabulary> - Report topics likely to be similar on the basis of their vocabulary

90 L<reportSourceFiles|/reportSourceFiles> - Source file for each topic

91 L<reportTableDimensions|/reportTableDimensions> - Report table dimensions

92 L<reportTables|/reportTables> - Report on tables that have problems

93 L<reportTagCount|/reportTagCount> - Report tag counts

94 L<reportTagsAndTextsCount|/reportTagsAndTextsCount> - Report tags and texts counts

95 L<reportTopicDetails|/reportTopicDetails> - Things that occur once in each file

96 L<reportTopicReuse|/reportTopicReuse> - Count how frequently each topic is reused

97 L<reportTopicsNotReferencedFromBookMaps|/reportTopicsNotReferencedFromBookMaps> - Topics not referenced from bookmaps

98 L<reportValidationErrors|/reportValidationErrors> - Report the files known to have validation errors

99 L<reportWordsByFile|/reportWordsByFile> - Index words to the files they occur in

100 L<reportXml1|/reportXml1> - Report bad xml on line 1

101 L<reportXml2|/reportXml2> - Report bad xml on line 2

102 L<testReferenceChecking|/testReferenceChecking> - Test reference checking

103 L<writeXrefStructure|/writeXrefStructure> - Write the test for an Xref structure

104 L<xref|/xref> - Check the cross references in a set of Dita files held in L<inputFolder|/inputFolder> and report the results in the L<reports|/reports> folder.

105 L<xref2|/xref2> - Check the cross references in a set of Dita files held in  L<inputFolder|/inputFolder> and report the results in the L<reports|/reports> folder.

106 L<xref22|/xref22> - Check the cross references in a set of Dita files held in L<inputFolder|/inputFolder> and report the results in the L<reports|/reports> folder.
