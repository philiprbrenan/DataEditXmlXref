...
sub xref22(%)                                                                     # Check the cross references in a set of Dita files held in L<inputFolder|/inputFolder> and report the results in the L<reports|/reports> folder. The possible attributes are defined in L<Data::Edit::Xml::Xref|/Data::Edit::Xml::Xref>.
 {my (%attributes) = @_;                                                        # Cross referencer attribute value pairs
  my $file = temporaryFile;
  if (my $pid = fork)                                                           # Parent: wait for child Xref to finish
   {waitpid $pid, 0;
    my $x = newXref(retrieveFile($file)->%*);                                   # Create LVALUE methods
    return $x;                                                                  # Return cross reference results
   }
  else                                                                          # Child: Xref in a separate process to avoid memory fragmentation in parent
   {my $x = xref2(%attributes);
    storeFile($file, $x);
    exit;
   }
 }
...
sub xref(%)                                                                     # Check the cross references in a set of Dita files held in L<inputFolder|/inputFolder> and report the results in the L<reports|/reports> folder. The possible attributes are defined in L<Data::Edit::Xml::Xref|/Data::Edit::Xml::Xref>.
 {my (%attributes) = @_;                                                        # Cross referencer attribute value pairs
  ...
  newXref %$x;                                                                  # Return blessed results
 }
...
sub analyzeOneFile($$)                                                          #P Analyze one input file
 {my ($Xref, $iFile) = @_;                                                      # Xref request, File to analyze
  my $xref = newXref($Xref->%*);                                                # Cross referencer for this file
...
sub analyzeInputFilesMergeResults($@)                                           #P Merge a list of cross reference results into the first cross referencer in the list
 {my ($xref, @x) = @_;                                                          # Cross referencer to merge into, other cross referencers
  ...
  newXref();                                                                    # Create methods if not already created
...
sub analyzeInputFiles($)                                                        #P Analyze the input files
 {my ($xref) = @_;                                                              # Cross referencer
  ...
  awsParallelProcessFiles
   ...
   \&analyzeInputFilesMergeResults,
    $xref->inputFiles,
    region=>awsCurrentRegion);                                                  # Assume other instances are in the same region
