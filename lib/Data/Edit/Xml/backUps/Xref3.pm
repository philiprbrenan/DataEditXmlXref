 ...
 {my $xref = newXref(@_);                                                       # Create the cross referencer
  ...
  if (!$xref->fixedFolder and $xref->fixDitaRefs || $xref->fixRelocatedRefs)    # Fixing references
   {my $s = join ' and ',
     ...
     confess "No fixedFolder attribute specified yet $s specified";
   }

  if (my $f = $xref->fixedFolder)                                               # Fixing references in a folder other thn the input fiolder requires us to copy fthe files across so we can make differential changes
...
#   $save->(1, "badXRefs",          q(files with bad xrefs), q(file with bad xrefs));
    ...
    say STDERR dateTimeStamp, " ", $statusLine;                                 # Print Xref results summary with date and time
...
#     };
    ...
    if (my $a = $o->{attributes})                                               # Count tags, attributes, attribute values
     {$countAttrNames{$_}++, $countAttrNamesAndValues{$_}{$$a{$_}}++ for keys %$a;
      $countTagNames{$tag}++;
     }
   });
...
sub analyzeInputFilesMergeResults($@)                                           #P Merge a list of cross reference results into the first cross referencer in the list
...
lll "field merge start $$ for ", scalar(@x);
...
mmm "field merge end $$ for ", scalar(@x);

lll "File merge start $$ for ", scalar(@x);
...
mmm "File merge end $$ for ", scalar(@x);
...
END
 ...
 } # analyzeInputFilesMergeResults

sub analyzeInputFileList($@)                                                    #P Analyze a list of input files. Return a list of cross referencers one for each input file analyzed.
 {my ($xref, @in) = @_;                                                         # Cross referencer
  my $x = newXref(%$xref);
  processFilesInParallel sub{analyzeOneFile($xref, $_[0])}, undef, @in;         # Analyze input files returning a list of cross referencers one for each input file analyzed
 }

sub analyzeInputFilesOneInstance($)                                             #P Analyze the input files on one instance
 {my ($xref) = @_;                                                              # Cross referencer
  my @x = analyzeInputFileList($xref, $xref->inputFiles->@*);                   # Cross reference for each input file analyzed

  analyzeInputFilesMergeResults($xref, @x);                                     # Merge cross referencers produced for each input file analyzed
 }

sub analyzeInputFilesEachInstance($$@)                                          #P Analyze a block of input files on an AWS instance
 {my ($xref, $resultsFolder, @files) = @_;                                      # Cross referencer, results folder, files to process.
  my $x = newXref(%$xref);                                                       # To make the lvalues subs work
  my @x = analyzeInputFileList ($x, @files);                                    # Cross referencer for each input file analyzed
  analyzeInputFilesMergeResults($x, @x);                                        # Merge cross referencers produced for each input file analyzed into main cross referencer
  storeFile(fpe($resultsFolder, awsCurrentIp, qw(results data)), $x);           # Save results from this instance in the results folder
 }

sub analyzeInputFilesMergeInstances($$)                                         #P Merge cross referencers from each instance
 {my ($xref, $resultsFolder) = @_;                                              # Cross referencer, results folder

  my @x;                                                                        # Cross referencer from each instance
  for my $file(searchDirectoryTreesForMatchingFiles($resultsFolder))            # Each results file
   {push @x, newXref(retrieveFile($file)->%*);                                      # Recreate Xref
   }

  analyzeInputFilesMergeResults($xref, @x);                                     # Merge cross referencers produced on each instance into main cross referencer
 }

sub analyzeInputFiles($)                                                        #P Analyze the input files
 {my ($xref) = @_;                                                              # Cross referencer

  my %o = (region=>awsCurrentRegion);                                           # Assume other instances are in the same region

  if (onAws and awsParallelIpAddressesOfOtherSessionInstances(%o))              # On AWS with more then one instance running
   {lll "Analyze files on multiple instances";
    awsParallelProcessFiles
     ($xref,
     \&analyzeInputFilesEachInstance,
     \&analyzeInputFilesMergeInstances,
      $xref->inputFiles,
      %o);
    mmm;
   }
  else                                                                          # Only one instance running
   {lll "Analyze files on one instance";
    analyzeInputFilesOneInstance($xref);
    mmm;
   }
 }
  ...
  if (my $out = $xref->subjectSchemeMap)                                        # SubjectSchemeMap requested by supplying an output folder for the maps
   {my $makeNavTitle = sub                                                      # Create a nav title using the Gearhart NavTitle Method
    ...
    for my $n(sort keys %o)                                                     # Layout map entries
     {next if $n =~ m(\Atopic_type\Z)i;                                         # sf260
      my $t = &$makeNavTitle($n);
...
END
      ...
      for   my $n(sort keys $xref->otherMeta->{$f}->%*)                         # Name of other meta in book map topic
       {next if $n =~ m(\Atopic_type\Z)i;                                       # sf260
        my $navTitle = $xref->otherMeta->{$f}{$n};                              # sf261
        push @o, <<END;
...
END
  ...
  my $resources = &$extractRefsAndWrapWithFolderName(q(conRefs), $out);         # Was file name before: sf250
...
15 L<createReportsInParallel|/createReportsInParallel> - Create reports in parallel

16 L<createReportsInParallel1|/createReportsInParallel1> - Create reports in parallel that do not require fixed references

17 L<createReportsInParallel2|/createReportsInParallel2> - Create reports in parallel that        require fixed references

18 L<createRequiredCleanUps|/createRequiredCleanUps> - Required clean ups report

19 L<createSampleConRefMatching|/createSampleConRefMatching> - Create sample input files for matching conref source and targets

20 L<createSampleConRefs|/createSampleConRefs> - Create sample input files for fixing a conref

21 L<createSampleDuplicateMd5Sum|/createSampleDuplicateMd5Sum> - Create sample input files with duplicate md5 sums

22 L<createSampleImageReferences|/createSampleImageReferences> - Good and bad image references

23 L<createSampleImageTest|/createSampleImageTest> - Create sample input files for fixing bookmap reference to a topic that did not get cut into  multiple pieces

24 L<createSampleInputFilesBaseCase|/createSampleInputFilesBaseCase> - Create sample input files for testing.

25 L<createSampleInputFilesFixFolder|/createSampleInputFilesFixFolder> - Create sample input files for testing fixFolder

26 L<createSampleInputFilesForFixDitaRefs|/createSampleInputFilesForFixDitaRefs> - Create sample input files for fixing renamed topic refs

27 L<createSampleInputFilesForFixDitaRefsImproved1|/createSampleInputFilesForFixDitaRefsImproved1> - Create sample input files for fixing references via the targets/ folder

28 L<createSampleInputFilesForFixDitaRefsImproved2|/createSampleInputFilesForFixDitaRefsImproved2> - Create sample input files for fixing conref references via the targets/ folder

29 L<createSampleInputFilesForFixDitaRefsImproved3|/createSampleInputFilesForFixDitaRefsImproved3> - Create sample input files for fixing bookmap references to topics that get cut into multiple pieces

30 L<createSampleInputFilesForFixDitaRefsImproved4|/createSampleInputFilesForFixDitaRefsImproved4> - Create sample input files for fixing bookmap reference to a topic that did not get cut into  multiple pieces

31 L<createSampleInputFilesForFixDitaRefsXref|/createSampleInputFilesForFixDitaRefsXref> - Create sample input files for fixing references into renamed topics by xref

32 L<createSampleInputFilesLtGt|/createSampleInputFilesLtGt> - Create sample input files for testing items between &lt; and &gt;

33 L<createSampleOtherMeta|/createSampleOtherMeta> - Create sample data for othermeta reports

34 L<createSampleTopicsReferencedFromBookMaps|/createSampleTopicsReferencedFromBookMaps> - The number of times a topic is referenced from a bookmap

35 L<createSampleUnreferencedIds|/createSampleUnreferencedIds> - Create sample input files with unreferenced ids

36 L<createSoftConrefs|/createSoftConrefs> - Fix file part of conref even if the rest is invalid

37 L<createTestOneNotRef|/createTestOneNotRef> - One topic refernced and the other not

38 L<createTestReferencedToFlattenedTopic|/createTestReferencedToFlattenedTopic> - Full reference to a topic that has been flattened

39 L<createTestReferenceToCutOutTopic|/createTestReferenceToCutOutTopic> - References from a topic that has been cut out to a topic that has been cut out

40 L<createTestTopicFlattening|/createTestTopicFlattening> - Create sample input files for testing topic flattening ratio reporting

41 L<createWordsToFilesTest|/createWordsToFilesTest> - Index words to file

42 L<editXml|/editXml> - Edit an xml file retaining any existing XML headers and lint trailers

43 L<externalReference|/externalReference> - Check for an external reference

44 L<fixFilesGB|/fixFilesGB> - Rename files to the L<GB Standard|http://metacpan.org/pod/Dita::GB::Standard>

45 L<fixingRun|/fixingRun> - A fixing run fixes problems where it can and thus induces changes which might make the updated output different from the incoming source.

46 L<fixOneFileGB|/fixOneFileGB> - Fix one file to the Gearhart-Brenan standard

47 L<fixReferences|/fixReferences> - Fix just the file containing references using a number of techniques and report those references that cannot be so fixed.

48 L<fixReferencesInOneFile|/fixReferencesInOneFile> - Fix one file by moving unresolved references to the xtrf attribute

49 L<formatTables|/formatTables> - Using cross reference B<$xref> options and an array of arrays B<$data> format a report as a table using B<%options> as described in L<Data::Table::Text::formatTable> and L<Data::Table::Text::formatHtmlTable>.

50 L<hashOfCountsToArray|/hashOfCountsToArray> - Convert a B<$hash> of {key} = count to an array so it can be formatted with L<formatTables>

51 L<loadInputFiles|/loadInputFiles> - Load the names of the files to be processed

52 L<newXref|/newXref> - Create a new cross referencer

53 L<oneBadRef|/oneBadRef> - Check one reference and return the first error encountered or B<undef> if no errors encountered.

54 L<oxygenProjectFileMetaData|/oxygenProjectFileMetaData> - Meta data for the oxygen project files

55 L<removeStringsFromData|/removeStringsFromData> - Given a data structure, remove the specified strings from it

56 L<removeUnusedIds|/removeUnusedIds> - Remove ids that do are not mentioned in any href or conref in the corpus regardless of the file component of any such reference.

57 L<reportAttributeCount|/reportAttributeCount> - Report attribute counts

58 L<reportAttributeNameAndValueCounts|/reportAttributeNameAndValueCounts> - Report attribute value counts

59 L<reportConRefMatching|/reportConRefMatching> - Report conref matching

60 L<reportDocTypeCount|/reportDocTypeCount> - Report doc type count

61 L<reportDuplicateIds|/reportDuplicateIds> - Report duplicate ids

62 L<reportDuplicateTopicIds|/reportDuplicateTopicIds> - Report duplicate topic ids

63 L<reportEmptyTopics|/reportEmptyTopics> - Report empty topics

64 L<reportExteriorMaps|/reportExteriorMaps> - Maps that are not referenced by any other map

65 L<reportExternalXrefs|/reportExternalXrefs> - Report external xrefs missing other attributes

66 L<reportFileExtensionCount|/reportFileExtensionCount> - Report file extension counts

67 L<reportFileTypes|/reportFileTypes> - Report file type counts - takes too long in series

68 L<reportFixRefs|/reportFixRefs> - Report of hrefs that need to be fixed

69 L<reportGuidHrefs|/reportGuidHrefs> - Report on guid hrefs

70 L<reportGuidsToFiles|/reportGuidsToFiles> - Map and report guids to files

71 L<reportHrefUrlEncoding|/reportHrefUrlEncoding> - href needs url encoding

72 L<reportIdRefs|/reportIdRefs> - Report the number of times each id is referenced

73 L<reportImages|/reportImages> - Reports on images and references to images

74 L<reportLtGt|/reportLtGt> - Report items found between &lt; and &gt;

75 L<reportMaxZoomOut|/reportMaxZoomOut> - Text located via Max Zoom In

76 L<reportMd5Sum|/reportMd5Sum> - Report files with identical md5 sums

77 L<reportNoHrefs|/reportNoHrefs> - Report locations where an href was expected but not found

78 L<reportOlBody|/reportOlBody> - ol under body - indicative of a task

79 L<reportOtherMeta|/reportOtherMeta> - Advise in the feasibility of moving othermeta data from topics to bookmaps assuming that the othermeta data will be applied only at the head of the map rather than individually to each topic in the map.

80 L<reportOtherMetaConsolidated|/reportOtherMetaConsolidated> - Create a subject scheme map from othermeta

81 L<reportParseFailed|/reportParseFailed> - Report failed parses

82 L<reportPublicIds|/reportPublicIds> - Report public ids in use

83 L<reportReferencesFromBookMaps|/reportReferencesFromBookMaps> - Topics and images referenced from bookmaps

84 L<reportRequiredCleanUps|/reportRequiredCleanUps> - Report required clean ups

85 L<reportSimilarTopicsByTitle|/reportSimilarTopicsByTitle> - Report topics likely to be similar on the basis of their titles as expressed in the non Guid part of their file names

86 L<reportSimilarTopicsByVocabulary|/reportSimilarTopicsByVocabulary> - Report topics likely to be similar on the basis of their vocabulary

87 L<reportSourceFiles|/reportSourceFiles> - Source file for each topic

88 L<reportTableDimensions|/reportTableDimensions> - Report table dimensions

89 L<reportTables|/reportTables> - Report on tables that have problems

90 L<reportTagCount|/reportTagCount> - Report tag counts

91 L<reportTagsAndTextsCount|/reportTagsAndTextsCount> - Report tags and texts counts

92 L<reportTopicDetails|/reportTopicDetails> - Things that occur once in each file

93 L<reportTopicReuse|/reportTopicReuse> - Count how frequently each topic is reused

94 L<reportTopicsNotReferencedFromBookMaps|/reportTopicsNotReferencedFromBookMaps> - Topics not referenced from bookmaps

95 L<reportValidationErrors|/reportValidationErrors> - Report the files known to have validation errors

96 L<reportWordsByFile|/reportWordsByFile> - Index words to the files they occur in

97 L<reportXml1|/reportXml1> - Report bad xml on line 1

98 L<reportXml2|/reportXml2> - Report bad xml on line 2

99 L<testReferenceChecking|/testReferenceChecking> - Test reference checking

100 L<writeXrefStructure|/writeXrefStructure> - Write the test for an Xref structure

101 L<xref|/xref> - Check the cross references in a set of Dita files held in  L<inputFolder|/inputFolder> and report the results in the L<reports|/reports> folder.
...
if (1)                                                                          # Check topic matching
 {clearFolder($_, 420) for in, out, reports;
  ...
  ok readFile(fpe($x->reports, qw(similar byVocabulary txt))) =~ m(1\s+8.*in/1\.dita);
 }
...
if (1)                                                                          # fixDitaRefs using target files to locate flattened files
 {clearFolder(tests, 111);
  ...
  my  $y = xref(inputFolder => outFixed);                                       # Check results
  ok !$y->errors;
 }
...
if (1)                                                                          # Soft conrefs
 {clearFolder(tests, 111);
  ...
  my $z = xref(inputFolder => outFixed);
