...
sub analyzeInputFiles22($)                                                      #P Analyze the input files
 {my ($xref) = @_;                                                              # Cross referencer

  if (onAws and awsParallelIpAddressesOfOtherSessionInstances)                  # On AWS with more then one instance running
   {awsParallelProcessFiles($xref, \&analyzeInputFiles33, sub{},
      $xref->inputFiles, $xref->awsOptions->%*);
   }
